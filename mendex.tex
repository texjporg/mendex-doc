...
\def\indexname{索引の例（\texttt{mendex}デフォルト）}
\InputIfFileExists{\jobname-sub-0.ind}{}{\typeout{Rerun!}}
\def\indexname{索引の例（\texttt{mendex -s jpbase}）}
\InputIfFileExists{\jobname-sub-1.ind}{}{\typeout{Rerun!}}
\def\indexname{索引の例（\texttt{mendex -s jpbase -g}）}
\InputIfFileExists{\jobname-sub-2.ind}{}{\typeout{Rerun!}}
\def\indexname{索引の例（スタイルに\texttt{priority 1}を追加）}
\InputIfFileExists{\jobname-sub-3.ind}{}{\typeout{Rerun!}}
\def\indexname{索引の例（スタイルに\texttt{character\_order "JESN"}を追加）}
\InputIfFileExists{\jobname-sub-4.ind}{}{\typeout{Rerun!}}
